            ...
            <div class="row align-item-center py-xl-5 py-sm-0 pt-5 mt-sm-0 mt-5">
                <div class="col-lg-6 col-sm-7 text-center text-sm-start pe-sm-0 pt-sm-0 pt-5 mt-sm-0 mt-5">
                    <div class="d-block pe-xl-4 pt-sm-0 pt-5 mw-sm-320 mx-sm-0 mx-auto">
                        ...
                        if (!empty($heading)) {
                            if ($heading != strip_tags($heading)) {
                                // is HTML
                                echo '<span class="h5-alt pt-sm-0 pt-4 mb-sm-2 mb-0">'.$heading.'</span>';
                            } else {
                                // not HTML
                        ...
                        if (!empty($subheading)) {
                            if ($subheading != strip_tags($subheading)) {
                                // is HTML
                                echo '<div class="border-bottom-tertiary h4-alt mb-2">'.$subheading.'</div>';
                            } else {
                                // not HTML
                        ...
                        if (!empty($description_html_allowed)) {
                            if ($description_html_allowed != strip_tags($description_html_allowed)) {
                                // is HTML
                                echo '<span class="p-alt mb-0 mb-lg-2 pe-lg-4 pe-xl-5">'.$description_html_allowed.'</span>';
                            } else {
                                // not HTML
